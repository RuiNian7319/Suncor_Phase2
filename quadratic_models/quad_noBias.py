...
min_max_normalization = MinMaxNormalization(np.concatenate([train_X, train_y], axis=1))
training_data = min_max_normalization(np.concatenate([train_X, train_y], axis=1))
testing_data = min_max_normalization(np.concatenate([test_X, test_y], axis=1))
...
test_y = testing_data[:, -1].reshape(-1, 1)
...
with tf.Session() as sess:
    ...
    for epoch in range(epochs):
        ...
        for i in range(total_batch_number):
            ...
            if i % 550 == 0:
                ...
                # Unnormalize data
                train_pred = np.multiply(train_pred, min_max_normalization.denominator[0, -1])
                train_pred = train_pred + min_max_normalization.col_min[0, -1]

                actual_labels = np.multiply(train_y, min_max_normalization.denominator[0, -1])
                actual_labels = actual_labels + min_max_normalization.col_min[0, -1]
                ...
                # Unnormalize data
                test_pred = np.multiply(test_pred, min_max_normalization.denominator[0, -1])
                test_pred = test_pred + min_max_normalization.col_min[0, -1]

                actual_labels = np.multiply(test_y, min_max_normalization.denominator[0, -1])
                actual_labels = actual_labels + min_max_normalization.col_min[0, -1]
    ...
    # Unnormalize data
    predictions = np.multiply(predictions, min_max_normalization.denominator[0, -1])
    predictions = predictions + min_max_normalization.col_min[0, -1]

    end_test_y = np.multiply(end_test_y, min_max_normalization.denominator[0, -1])
    end_test_y = end_test_y + min_max_normalization.col_min[0, -1]

    # RMSE & MAE Calc
    RMSE_loss = np.sqrt(np.mean(np.square(np.subtract(end_test_y, predictions))))
    MAE_loss = np.mean(np.abs(np.subtract(end_test_y, predictions)))

    print('RMSE: {} | MAE: {}'.format(RMSE_loss, MAE_loss))

    # Visualization of what it looks like
    stdErr = np.std(np.abs(np.subtract(end_test_y, predictions)))

    group1 = np.concatenate([np.linspace(0, 999, 1000).reshape(-1, 1), predictions[0:1000]], axis=1)
    group2 = np.concatenate([np.linspace(0, 999, 1000).reshape(-1, 1), predictions[0:1000] + stdErr], axis=1)
    group3 = np.concatenate([np.linspace(0, 999, 1000).reshape(-1, 1), predictions[0:1000] - stdErr], axis=1)
