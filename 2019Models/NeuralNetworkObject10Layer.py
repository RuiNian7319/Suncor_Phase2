...
class NeuralNetwork:
   ...
   weights_and_biases: Returns the weights and biases of the model
    ...
    def __init__(self, session, train_x, train_y, test_x, test_y, lr=0.003, minibatch_size=2048, train_size=0.9,
                 epochs=5, lambd=0.001, h1_nodes=30, h2_nodes=30, h3_nodes=30, h4_nodes=30, h5_nodes=30, h6_nodes=30,
                 h7_nodes=30, h8_nodes=30, h9_nodes=30):
        ...
        self.h4_nodes = h4_nodes
        self.h5_nodes = h5_nodes
        self.h6_nodes = h6_nodes
        self.h7_nodes = h7_nodes
        self.h8_nodes = h8_nodes
        self.h9_nodes = h9_nodes
        self.output_size = 1
        ...
        with tf.name_scope('Model'):
            ...
            self.hidden_layer4 = {'W': tf.get_variable('h4_weights', shape=[self.h3_nodes, self.h4_nodes],
                                                       initializer=tf.contrib.layers.variance_scaling_initializer()),
                                  'b': tf.get_variable('h4_bias', shape=[self.h4_nodes],
                                                       initializer=tf.contrib.layers.variance_scaling_initializer())}

            self.hidden_layer5 = {'W': tf.get_variable('h5_weights', shape=[self.h4_nodes, self.h5_nodes],
                                                       initializer=tf.contrib.layers.variance_scaling_initializer()),
                                  'b': tf.get_variable('h5_bias', shape=[self.h5_nodes],
                                                       initializer=tf.contrib.layers.variance_scaling_initializer())}

            self.hidden_layer6 = {'W': tf.get_variable('h6_weights', shape=[self.h5_nodes, self.h6_nodes],
                                                       initializer=tf.contrib.layers.variance_scaling_initializer()),
                                  'b': tf.get_variable('h6_bias', shape=[self.h6_nodes],
                                                       initializer=tf.contrib.layers.variance_scaling_initializer())}

            self.hidden_layer7 = {'W': tf.get_variable('h7_weights', shape=[self.h6_nodes, self.h7_nodes],
                                                       initializer=tf.contrib.layers.variance_scaling_initializer()),
                                  'b': tf.get_variable('h7_bias', shape=[self.h7_nodes],
                                                       initializer=tf.contrib.layers.variance_scaling_initializer())}

            self.hidden_layer8 = {'W': tf.get_variable('h8_weights', shape=[self.h7_nodes, self.h8_nodes],
                                                       initializer=tf.contrib.layers.variance_scaling_initializer()),
                                  'b': tf.get_variable('h8_bias', shape=[self.h8_nodes],
                                                       initializer=tf.contrib.layers.variance_scaling_initializer())}

            self.hidden_layer9 = {'W': tf.get_variable('h9_weights', shape=[self.h8_nodes, self.h9_nodes],
                                                       initializer=tf.contrib.layers.variance_scaling_initializer()),
                                  'b': tf.get_variable('h9_bias', shape=[self.h9_nodes],
                                                       initializer=tf.contrib.layers.variance_scaling_initializer())}

            self.output_layer = {'W': tf.get_variable('output_weights', shape=[self.h9_nodes, self.output_size],
                                                      initializer=tf.contrib.layers.variance_scaling_initializer()),
        ...
        self.l4 = tf.add(tf.matmul(self.l3, self.hidden_layer4['W']), self.hidden_layer4['b'])
        self.l4 = tf.nn.relu(self.l4)
        self.l4 = tf.layers.batch_normalization(self.l4, training=self.training)

        self.l5 = tf.add(tf.matmul(self.l4, self.hidden_layer5['W']), self.hidden_layer5['b'])
        self.l5 = tf.nn.relu(self.l5)
        self.l5 = tf.layers.batch_normalization(self.l5, training=self.training)

        self.l6 = tf.add(tf.matmul(self.l5, self.hidden_layer6['W']), self.hidden_layer6['b'])
        self.l6 = tf.nn.relu(self.l6)
        self.l6 = tf.layers.batch_normalization(self.l6, training=self.training)

        self.l7 = tf.add(tf.matmul(self.l6, self.hidden_layer7['W']), self.hidden_layer7['b'])
        self.l7 = tf.nn.relu(self.l7)
        self.l7 = tf.layers.batch_normalization(self.l7, training=self.training)

        self.l8 = tf.add(tf.matmul(self.l7, self.hidden_layer8['W']), self.hidden_layer8['b'])
        self.l8 = tf.nn.relu(self.l8)
        self.l8 = tf.layers.batch_normalization(self.l8, training=self.training)

        self.l9 = tf.add(tf.matmul(self.l8, self.hidden_layer9['W']), self.hidden_layer9['b'])
        self.l9 = tf.nn.relu(self.l9)
        self.l9 = tf.layers.batch_normalization(self.l9, training=self.training)

        self.output = tf.add(tf.matmul(self.l9, self.output_layer['W']), self.output_layer['b'])
...
def train_model(data_path, model_path, norm_path, test_size=0.05, shuffle=True, lr=0.003, minibatch_size=2048,
                epochs=30, lambd=0.001, h1_nodes=3000, h2_nodes=3000, h3_nodes=3000, h4_nodes=3000, h5_nodes=3000,
                h6_nodes=3000, h7_nodes=3000, h8_nodes=3000, h9_nodes=30,
    ...
    with tf.Session() as sess:
        ...
        nn = NeuralNetwork(sess, train_x, train_y, test_x, test_y, lr=lr, minibatch_size=minibatch_size,
                           ...
                           h4_nodes=h4_nodes, h5_nodes=h5_nodes, h6_nodes=h6_nodes, h7_nodes=h7_nodes,
                           h8_nodes=h8_nodes, h9_nodes=h9_nodes,
...
if __name__ == "__main__":
    ...
    Raw_data, Heading_names, NN = train_model(Data_path, Model_path, Norm_path, test_size=0.01,
                                              shuffle=True, lr=0.001, minibatch_size=16000,
