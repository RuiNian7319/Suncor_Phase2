...
def train_model(data_path, model_path, norm_path, test_size=0.05, shuffle=True, lr=0.003, minibatch_size=2048,
    ...
    with tf.Session() as sess:
        ...
        if testing:
            ...
            # Evaluate loss
            rmse, mae = nn.eval_loss(pred, actual_y)
            ...
            # Auto correlation
            plt.acorr(residuals[:, 0], maxlags=1000)
            plt.show()

            # Cross correlation
            plt.xcorr(pred[:, 0], actual_y[:, 0], maxlags=1000)
            plt.show()
